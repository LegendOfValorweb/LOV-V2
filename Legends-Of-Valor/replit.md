...
- **Layout & Navigation Overhaul** (May 2026): Added Whiteout Survival-style bottom navigation bar (Map | Tower | Base | Army | Quests | More) — always visible across all pages, gold-themed with active tab indicator stripe; player panel and companion panel raised 62px to clear the nav bar; menu popup repositioned to open upward (bottom: 58px) instead of downward; 14 missing routes added to ZONE_NAMES (barracks, skill-tree, skill-workshop, dimensions, shadow-echoes, prestige, traits, ancient-ruins, enchanted-forest, quest-board, casino, research-lab, black-market, combat-log)
- **World Map Zone Overlap Fix** (May 2026): Eliminated all overlapping zone buttons — research-lab and skill-workshop were on exact same coordinates (35,72); hall-of-legends was 3% from mystic-tower; hell-zone clustered with both; tournament-grounds and mountain-caverns shared Y=22; shadow-realm and pet-arena were 2% apart. Fixed 8 zone coordinates to maintain minimum 10% separation. Selected zone detail panel raised to bottom:62px to clear bottom nav.
- **Tutorial Overhaul** (May 2026): Replaced 9 tutorial steps to cover new features — added Rank Progression & Power step (explains 0–96% rank stat bonus, gold scaling up to 6×); added Army & War Front step (Barracks at Expert rank, troop types, hero stat bonuses); updated combat/tower/quest steps to reference bottom nav; all action prompts updated to use new bottom nav bar labels
